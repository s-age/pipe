...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function toggleEdit(editButton, sessionId, turnIndex, turnType) {
    ...
    if (editableElement.previousElementSibling)
      editableElement.previousElementSibling.style.display = 'none'
    ...
    fetch(`/api/session/${sessionId}/turn/${turnIndex}/edit`, {
      ...
      .then((data) => {
        if (data.success) {
          const turnElement = document.getElementById(`turn-${turnIndex}`)
          const turnType =
            turnElement.classList.contains('model_response') ||
            turnElement.classList.contains('condensed_history')
              ? 'model_response'
              : 'user_task'
  ...
  function saveMetaEdit(sessionId) {
    ...
    const payload = {
      ...
      roles: viewContainer
        .querySelector('[data-field="roles"]')
        .value.split(',')
        .map((s) => s.trim())
        .filter(Boolean),
  ...
  function createTurnElement(turn, index, expertMode) {
    ...
    const timestamp = turn.timestamp
      ? turn.timestamp.replace('T', ' ').split('.')[0]
      : ''
  ...
  function attachEventListenersToTurns() {
    ...
    document.querySelectorAll('.copy-turn-btn').forEach((button) => {
      button.addEventListener('click', function (e) {
        ...
        const turnElement = document.getElementById(`turn-${turnIndex}`)
        const turnType = turnElement.classList.contains('model_response')
          ? 'model_response'
          : turnElement.classList.contains('condensed_history')
            ? 'condensed_history'
            : 'user_task'
  ...
  document
    ...
    .addEventListener('click', () => (window.location.href = '/new_session'))
