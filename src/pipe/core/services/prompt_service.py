...
class PromptService:
    ...
    def build_prompt(self, session_service: SessionService) -> Prompt:
        """
        Builds and returns a validated Prompt object based on the current session
        and settings.
        """
        ...
        roles = PromptRoles(
            ...
            definitions=RoleCollection(session_data.roles).get_for_prompt(
                self.project_root
            ),
        )
        ...
        references_with_content = list(
            ReferenceCollection(session_data.references).get_for_prompt(
                self.project_root
            )
        )
        prompt_references = [
