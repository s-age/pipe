...
class VerificationService:
    ...
    def _run_verifier_agent(
        ...
        verifier_instruction = (
            ...
            "Judge if this summary is acceptable for preserving "
            "conversational context.\n\n"
            "## STEP-BY-STEP INSTRUCTIONS\n\n"
            "**Step 1: Retrieve your own session data**\n"
            "Call the get_session tool with this exact parameter:\n"
            "```\n"
            f'get_session(session_id="{verifier_session_id}")\n'
            "```\n\n"
            "**Step 2: Analyze the conversation with focus on critical information**\n"
            "Read the entire conversation history. The 'compressed_history' turn "
            "replaces the original content. Focus on these critical aspects:\n\n"
            "1. **Essential context preservation**: Does the summary preserve "
            "information that is actually referenced or needed in subsequent "
            "turns? Minor omissions are acceptable if they don't affect "
            "understanding.\n\n"
            "2. **High-attention information retention**: Your model has "
            "attention mechanisms that assign importance scores to different "
            "parts of the conversation. Verify that information with high "
            "attention scores (information that was heavily referenced or "
            "central to the discussion) is retained in the summary. It's "
            "acceptable to omit low-importance details.\n\n"
            "3. **Conversational flow**: Does the conversation flow naturally "
            "from the turns before the summary, through the summary, to the "
            "turns after? Minor wording differences are acceptable as long "
            "as the logical flow is preserved.\n\n"
            "**Step 3: Make a judgment**\n"
            "Apply a PRAGMATIC standard:\n"
            "- **Approve** if the summary adequately preserves critical "
            "information and conversational flow, even if minor details "
            "differ or are simplified.\n"
            "- **Reject** ONLY if there are SIGNIFICANT issues: critical "
            "information loss, high-attention information omitted, or severe "
            "logical flow disruption.\n\n"
            "Do NOT reject for:\n"
            "- Minor wording differences\n"
            "- Simplified explanations of the same concept\n"
            "- Omission of redundant or low-importance details\n"
            "- Stylistic variations\n\n"
            "**Step 4: Generate Response**\n"
            "Based on your judgment, you MUST generate a response that starts with "
            "ONE of the following two prefixes: `Approved:` or `Rejected:`. Do not "
            "add any other text, explanation, or markdown fences before the prefix.\n\n"
            "## OUTPUT FORMATS\n\n"
            f"### Format 1: If your judgment is 'Approved'\n"
            f"Your entire response must start with the exact phrase below.\n"
            f"`Approved: {approved_text}`\n\n"
            f"### Format 2: If your judgment is 'Rejected' (only for SIGNIFICANT "
            f"issues)\n"
            "Your entire response must start with the exact phrase below, "
            "followed by a clear explanation focusing on CRITICAL problems only.\n"
            f"`Rejected: {rejected_text}`\n"
            "## REJECTED REASON\n\n"
            "[Explain ONLY the critical issues that prevent approval]"
        )
