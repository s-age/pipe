...
def read_file(absolute_path: str, limit: Optional[float] = None, offset: Optional[float] = None, session_service=None, session_id=None) -> Dict[str, str]:
    """
    ...
    try:
        ...
        if os.path.getsize(abs_path) == 0:
            message = f"File '{abs_path}' has been added or updated in session references, but it is empty."
        else:
            message = f"File '{abs_path}' has been added or updated in session references."
