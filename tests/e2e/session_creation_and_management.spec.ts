...
test.describe.serial('Session Creation and Management', () => {
  ...
  test('should create, fork, create child, modify, and delete sessions', async ({ page }) => {
    ...
    await page.waitForURL(url => url.toString() !== originalSessionUrl, { timeout: 60000 });
  ...
  test('should create a new session with various parameters, verify initial response, and edit meta-information', async ({ page }) => {
    await page.goto(NEW_SESSION_PAGE);
    ...
    await expect(page.locator('[data-field="background"]')).toHaveValue(updatedBackground);
    await expect(page.locator('[data-field="roles"]')).toHaveValue(updatedRoles);
    await expect(page.locator('[data-field="temperature"]')).toHaveValue(updatedTemperature);
  ...
  test('should fork an existing session at a specific turn', async ({ page }) => {
    ...
    await page.locator('textarea[name="background"]').fill('This session is created to test forking functionality.');
    await page.locator('input[name="roles"]').fill('roles/engineer.md');
    await page.locator('input[name="references"]').fill('README.md');
    await page.locator('textarea[name="instruction"]').fill('Initial instruction for fork test.');
    await page.locator('button[type="submit"]').click();
    ...
    expect(forkedSessionUrl).toMatch(/http:\/\/127\.0\.0\.1:5001\/session\/[a-f0-9]{64}/);
    await expect(page.locator('#turns-column h2')).toContainText('Fork of: Original Session for Fork Test');
