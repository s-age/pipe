...
class TestPromptService(unittest.TestCase):
    ...
    def test_build_prompt_basic_structure(self):
        ...
        args = TaktArgs(purpose="Test Purpose", background="Test BG", instruction="Test Instruction")
    ...
    def test_build_prompt_with_history(self):
        ...
        args = TaktArgs(purpose="Test", background="Test", instruction="Third instruction")
        self.session_service.prepare_session_for_takt(args)
        session_id = self.session_service.current_session_id

        # Manually add older turns to simulate a history
        session = self.session_service.get_session(session_id)
        session.turns.insert(0, ModelResponseTurn(type="model_response", content="Response to first", timestamp="..."))
        session.turns.insert(0, TaktArgs(instruction="First instruction").to_turn("...")) # Simulate another user turn
        ...
        self.assertEqual(prompt.conversation_history.turns[0]['instruction'], "First instruction")
        self.assertEqual(prompt.conversation_history.turns[1]['content'], "Response to first")
    ...
    def test_build_prompt_with_references_and_todos(self):
        ...
        args = TaktArgs(purpose="Test", background="Test", instruction="Test", references=["ref.txt"])
        self.session_service.prepare_session_for_takt(args)
        session_id = self.session_service.current_session_id
    ...
    def test_build_prompt_multi_step_reasoning(self):
        ...
        args_enabled = TaktArgs(
            purpose="Test", background="Test", instruction="Test", multi_step_reasoning=True
        )
        ...
        self.assertTrue(prompt_enabled.constraints.processing_config.multi_step_reasoning_active)
        ...
        args_disabled = TaktArgs(
            purpose="Test", background="Test", instruction="Test", multi_step_reasoning=False
        )
        ...
        self.assertFalse(prompt_disabled.constraints.processing_config.multi_step_reasoning_active)
