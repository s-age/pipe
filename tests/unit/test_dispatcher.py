...
class TestDispatcher(unittest.TestCase):
    ...
    def test_dispatch_routes_to_help_delegate(self, mock_help_run):
        ...
        def test_dispatch_run_handles_dry_run(self, mock_gemini_run, mock_dry_run):
            ...
            self.session_service.prepare_session_for_takt(
                args, is_dry_run=True
            )  # Prepare the service state
        ...
        def test_dispatch_run_decrements_ttl(self, mock_gemini_run):
            """Tests that _dispatch_run calls the TTL decrement method on a normal
            run."""
        ...
        def test_dispatch_run_does_not_decrement_ttl_on_dry_run(self, mock_dry_run):
            """Tests that _dispatch_run does NOT call the TTL decrement method on a
            dry run."""
        ...
        def test_ttl_and_expiration_are_called(self, mock_gemini_run):
            """Tests that both TTL decrement and tool response expiration are called on
            a normal run."""
