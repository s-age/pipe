...
class TestHistoryService(unittest.TestCase):
    ...
    def test_add_and_get_turn(self):
        ...
        self.history_service.add_turn_to_session(self.session_id, turn_data)
        
        turns = self.history_service.get_session_turns(self.session_id)
    ...
    def test_update_turns(self):
        ...
        self.history_service.add_turn_to_session(self.session_id, initial_turn)
        
        new_turns = [
            {"type": "model_response", "content": "response 1"},
            {"type": "user_task", "instruction": "next task"}
        ]
        self.history_service.update_turns(self.session_id, new_turns)
        
        retrieved_turns = self.history_service.get_session_turns(self.session_id)
    ...
    def test_delete_turn(self):
        ...
        for turn in turns_to_add:
            self.history_service.add_turn_to_session(self.session_id, turn)
            
        self.history_service.delete_turn(self.session_id, 1) # Delete the second turn
        
        remaining_turns = self.history_service.get_session_turns(self.session_id)
